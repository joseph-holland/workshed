#!/usr/bin/env python
# -*- coding: utf-8 -*-
# Colors from http://upload.wikimedia.org/wikipedia/en/1/15/Xterm_256color_chart.svg
...
class Powerline:
    ...
    def __init__(self, args, cwd):
        ...
        self.transparent_bg = self.color_template % '[49m'
    ...
    def bgcolor(self, code):
        # return self.color('49', code) if code == -1 else self.color('48', code)
...
class DefaultColor:
    ...
    STS_TOKEN_BG_GOOD = 22
    STS_TOKEN_BG_BAD = 124
    STS_TOKEN_FG = 15
    ...
    REPO_CLEAN_FG = 16  # black
    REPO_DIRTY_BG = 202  # yellow
    REPO_DIRTY_FG = 16  # back
...
def add_time_segment():
    ...
    #    else:
    import time
    time = '[%s] ' % time.strftime('%a %b %d %H:%M:%S')
...
def add_timediff_segment():
    ...
    if diff != '0':
        ...
        human_readable = lambda delta: [
            '%d%.1s' % (getattr(delta, attr), getattr(delta, attr) > 1 and attr or attr[:-1])
...
def add_aws_sts_token_segment():
    # Check for the env variable
    aws_sts_token = os.getenv('AWS_TOKEN_EXPIRY', None)

    if aws_sts_token:
        date_now = datetime.fromtimestamp(time.mktime(time.localtime()))
        date_expiry = datetime.fromtimestamp(time.mktime(time.localtime(float(aws_sts_token))))
        remaining_time = date_expiry - date_now

        if date_expiry > date_now:
            powerline.append(' %s ' % str(remaining_time), Color.STS_TOKEN_FG, Color.STS_TOKEN_BG_GOOD)
        else:
            powerline.append(' EXPIRED ', Color.STS_TOKEN_FG, Color.STS_TOKEN_BG_BAD)

            # print remaining_time


add_aws_sts_token_segment()

import socket


def add_user_segment():
    user_name = os.getenv('USER')
    host = socket.gethostname().split('.')[0]

    aws_id = os.getenv('AWS_ID_NAME', None)
    aws_region = os.getenv('AWS_DEFAULT_REGION', None)

    if aws_id:
        user_prompt = ' %s@%s ' % (aws_id, aws_region)
    else:
        user_prompt = '%s@%s ' % (user_name, host)
...
def add_path_segment():
    ...
    path_prompt = ' %s ' % (path)
    powerline.append(path_prompt, Color.PATH_FG, Color.PATH_BG)
